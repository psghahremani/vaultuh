# Vaultuh
Vaultuh is a simple offline encrypted vault. Vaultuh is written in Go and delegates the (symmetric) encryption to GPG.
...
### Overall Flow
To start things off, you'll need to ***create a new encrypted vault***. The encrypted vault is stored as a simple ***GPG encrypted file***. Once you have an encrypted vault, you are free to ***store it under any path and name*** that you may like. You can ***safely take regular cloud backups*** of the vault (***use a strong password***). The content of the vault can be edited at anytime in order to ***add/remove crendentials***. The vault's ***password can also be changed*** at anytime. Credentials can be fetched with the help fuzzy searching with the option to be either directly ***printed on your terminal*** or simply ***copied into the clipboard***.
...
#### Hidden Keys
As mentioned before, you may mark some keys to be hidden when you add them to your vault. Upon reading from your vault, by selecting the ```_``` option in the list of decrypted fields, your password
...
Use ```{EDITOR=my_favorite_editor} vaultuh update data [path]``` and enter password. This will decrypt your vault and open an editor that allows you to modify the decrypted content. Once you're done with your edits, save the file and exit the editor. By default, ```vi``` is used as the editor, but you can provide your custom editor using optional ```EDITOR``` environment variable (as shown in the provided example).
...
- You can't edit you vault using editors that operate outside the commandline (such as Visual Studio Code). 
